...
End;
Begin
     // appel au manager de ressources pour �viter de charger une texture d�j� charg�e
...
Begin
     ...
     // appel au manager de ressources pour �viter de charger un mesh d�j� charg�
     ...
     // envoie la g�om�trie dans la m�moire vid�o
...
//////////////////////////////////////////////////////////////////////////////////////////
// AddAnimation : Charge un OGLAnimation depuis un fichier (*.A12), l'ajoute � la pile  //
//                de donn�es, puis renvoie son pointeur.                                //
//////////////////////////////////////////////////////////////////////////////////////////
Function AddAnimation ( sFile : String ; nIndex : LongInt ; nMeshIndex : LongInt ; bRe : Boolean ) : LPOGLAnimation ;
...
Begin
     ...
     // appel au manager de ressources pour �viter de charger une animation d�j� charg�
     ...
     // cr�ation du pointeur vers la nouvelle animation
     New( pAnimation );

     // appel au manager de ressources pour r�cup�rer le mesh associ�
     ...
     // allocation de la m�moire pour le tableau d'actions
     pAnimation^.ActionCount := nSize;
     SetLength( pAnimation^.ActionArray, nSize );

     // allocation de la m�moire pour le tableau de vecteurs temporaires
     ...
     // allocation de la m�moire pour le tableau de frames
     ...
     For i := 0 To nSize - 1 Do
     Begin
          // allocation de la m�moire pour le tableau de vecteurs de la frame
          SetLength( pAnimation^.FrameArray[i].VectorArray, pMesh^.VertexCount );

          // lecture de la dur�e de la frame
          Assign( ioLong, sFile );
          Reset( ioLong, 4 );
          Seek( ioLong, j ); j += 1;
     ...
     // ajout de l'animation � la pile de donn�es
...
///////////////////////////////////////////////////////////////////////////////////////////
// DrawAnimation : Proc�de au rendu d'une OGLAnimation en fonction de son indice.        //
///////////////////////////////////////////////////////////////////////////////////////////
Procedure DrawAnimation ( nIndex : LongInt ; t : Boolean ; nAction : LongInt ) ;
Var pAnimation : LPOGLAnimation ;
    ...
    p : QWord ;
Begin
     // recherche de l'animation � afficher en fonction de son indice
     ...
     // d�termination de la frame en cours et de la suivante
     ...
     // interpolation lin�aire entre deux frames
...
Var pAnimation : LPOGLAnimation;
Begin
     // recherche de l'animation � s�lectionner en fonction de son indice
     pAnimation := FindItem( DATA_ANIMATION, nIndex );
     If pAnimation = NIL Then Exit;
...
////////////////////////////////////////////////////////////////////////////////
// InitShaderProgram : Charge le shader en fonction du mod�le.                //
////////////////////////////////////////////////////////////////////////////////
Procedure InitShaderProgram () ;
...
Begin
     ...
     // d�finit les techniques de rendu
     glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
     glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
     glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
End;
...
Begin
     // appel au manager de ressources pour �viter de charger un son d�j� charg�
