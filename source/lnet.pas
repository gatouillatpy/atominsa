...
uses
  {$ifndef MSWINDOWS} BaseUnix, NetDB, {$else} TomWinsock, {$endif}
...
type
  ...
  TLSocket = class(TLBaseSocket)
   protected
    ...
    FSnum: Integer;
    FCanSend: Boolean;
    FCheckSend: Boolean;   // this is for automagical sending too
    FCanReceive: Boolean;
  ...
  TLConnection = class(TComponent, ILServer, ILClient)
   protected
    ...
    procedure ConnectAction(var aSocket: TLSocket); virtual;
    procedure AcceptAction(var aSocket: TLSocket); virtual;
    procedure ReceiveAction(var aSocket: TLSocket); virtual;
    procedure SendAction(var aSocket: TLSocket); virtual;
    procedure ErrorAction(var aSocket: TLSocket); virtual;
  ...
  TLUdp = class(TLConnection)
   protected
    ...
    procedure ReceiveAction(var aSocket: TLSocket); override;
    procedure SendAction(var aSocket: TLSocket); override;
    procedure ErrorAction(var aSocket: TLSocket); override;
  ...
  TLTcp = class(TLConnection)
   protected
    ...
    procedure ConnectAction(var aSocket: TLSocket); override;
    procedure AcceptAction(var aSocket: TLSocket); override;
    procedure ReceiveAction(var aSocket: TLSocket); override;
    procedure SendAction(var aSocket: TLSocket); override;
    procedure ErrorAction(var aSocket: TLSocket); override;
  ...
  function fpFD_ISSET(const Socket: Integer; var FDSet: TFDSet): Integer;
  procedure fpFD_SET(const Socket: Integer; var FDSet: TFDSet);
...
procedure TLConnection.ConnectAction(var aSocket: TLSocket);
begin

end;

procedure TLConnection.AcceptAction(var aSocket: TLSocket);
begin

end;

procedure TLConnection.ReceiveAction(var aSocket: TLSocket);
begin

end;

procedure TLConnection.SendAction(var aSocket: TLSocket);
begin

end;

procedure TLConnection.ErrorAction(var aSocket: TLSocket);
begin
...
var
  ...
  TempVal: TTimeVal;
begin
  if not FSerSock.FCanReceive  then begin
    TempVal:=FTimeVal;
    fpFD_ZERO(RSet);
    fpFD_ZERO(ESet);
    fpFD_ZERO(WSet);
    ...
    if not FSerSock.FCanSend or FSerSock.FCheckSend then
    ...
    if n > 0 then begin
      ...
      and (fpFD_ISSET(FSerSock.FHandle, ESet) = 0) then
        ReceiveAction(FSerSock);

      if FSerSock.Connected
      and (fpFD_ISSET(FSerSock.FHandle, ESet) <> 0) then
        ErrorAction(FSerSock);
    end;
  end;
end;

procedure TLUdp.ReceiveAction(var aSocket: TLSocket);
begin
  aSocket.FCanReceive:=True;
  if Assigned(FOnReceive) then
    FOnReceive(FSerSock);
end;

procedure TLUdp.SendAction(var aSocket: TLSocket);
begin
  aSocket.FCanSend:=True;
  aSocket.FCheckSend:=False;
  if Assigned(FOnCanSend) then
    FOnCanSend(FSerSock);
end;

procedure TLUdp.ErrorAction(var aSocket: TLSocket);
begin
  Bail('Error');
end;
...
begin
  Result:=FSerSock.GetMessage(msg);
end;
...
procedure TLTcp.ConnectAction(var aSocket: TLSocket);
var
  {$ifdef MSWINDOWS}
  a: TSockAddrin;
  {$else}
...
procedure TLTcp.AcceptAction(var aSocket: TLSocket);
begin
...
procedure TLTcp.ReceiveAction(var aSocket: TLSocket);
begin
...
procedure TLTcp.SendAction(var aSocket: TLSocket);
begin
  aSocket.FCanSend:=True;
  aSocket.FCheckSend:=False;
  if Assigned(FOnCanSend) then
...
procedure TLTcp.ErrorAction(var aSocket: TLSocket);
begin
  if aSocket.Connecting then
    Bail('Error on connect: connection refused' , aSocket)
  else
    Bail('Error', aSocket);
end;
...
function TLTcp.CallSelect: Boolean;
var
  i: Integer;
  max: Integer;
  TempVal: TTimeVal;
  ...
  if Assigned(FSerSock.Next) then begin
    ...
    repeat
      if Tmp.Connected or Tmp.Connecting then begin
        if (not Tmp.FCanReceive) or (not Tmp.FCanSend) or Tmp.FCheckSend then begin
          if not Tmp.FCanReceive then
            fpFD_SET(Tmp.FHandle, FReadFDSet);
          if not Tmp.FCanSend or Tmp.FCheckSend then
...
begin
  ...
  fpFD_ZERO(FFDSet);
  fpFD_SET(FHandle, FFDSet);
  if fpSelect(FHandle+1, @FFDSet, nil, nil, nil) < 0 then
    Bail('Error on select', SocketError)
  else begin
    if fpFD_ISSET(FHandle, FFDSet) <> 0 then
      Result:=True and FConnected;
...
begin
  ...
  if CanSend then begin
    ...
    if Result <= 0 then begin
      if SocketError = BLOCK_ERROR then begin
        FCanSend:=False;
        FCheckSend:=True;
        Result:=0;
      end else
        Bail('Send error', SocketError);
    end else FCheckSend:=True;
  end;
end;
...
begin
  ...
  with FParent do case anAction of
    acConnect : ConnectAction(TempSelf);
    acAccept  : AcceptAction(TempSelf);
    acSend    : SendAction(TempSelf);
    acReceive : ReceiveAction(TempSelf);
    acError   : ErrorAction(TempSelf);
  end;
end;
